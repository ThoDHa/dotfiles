...
Planning labor (exploration, analysis, drafting task documentation, proposing a breakdown) MAY be delegated; planning approval NEVER is. When work is tracked in task files and the planned breakdown fans out into parallel children with overlapping territory, the planning labor MUST satisfy the `task-files` skill's shared-reconnaissance default (deposit the shared reconnaissance artifact under the parent or record the Decision Log exception), defined in that skill's Triage to Ready Planning Phase. The manager is the sole agent-side authority that approves a planned task for execution: approval is never delegated to an agent or ally, and it never substitutes for the user's sign-off under the [Execution Sign-off Gate](#execution-sign-off-gate). Implementations MUST NOT allow a delegated agent or ally to declare its own planning complete and move work into execution; when agents performed the planning labor, the manager MUST review the resulting plan, verify it is complete and aligned, and grant approval personally. Delegation does not absolve accountability. When the task is tracked in a task file, this approval is the Triage → Ready transition (see the `task-files` skill).

### Execution Sign-off Gate

Agent-side planning approval never substitutes for the user's. Before the first execution dispatch of a task (the first worker dispatch in Delegating mode, the first work step beyond planning in Solo), the manager MUST present the plan to the user and wait for explicit go-ahead. The gate applies to every task in both modes regardless of size; a user-granted standing waiver is honored until revoked.

The presented plan MUST be scaled to the task: at minimum the objective reading, the unit breakdown with territories, the execution order (which units run sequentially, in parallel, and in worktrees), and the verification approach; when the task admits multiple viable approaches, the plan MUST also present the alternatives and their tradeoffs. Any then-pending Significant questions MUST be batched into the same presentation per Question Batching Discipline.

One sign-off covers its plan: the units it contains, their fix, verification, and review rounds, retries, restarts of parked tasks from that plan, and backfill dispatches. Genuinely new work (a task raised after completion, a deferred suggestion picked up, a new user request) gets its own gate. A material deviation from the approved plan (new territory, scope growth, a different execution shape) is a new plan and returns to the gate before dispatch.

Planning labor is not execution: planner and research dispatches MAY run before the gate, and when task files are active the Triage → Ready transition precedes it. The `task-files` skill's checkpoint sign-off gating mode is a separate, per-checkpoint stop for sliced tasks: neither gate implies the other, though a plan MAY carry both.
...
**Standing restart grant.** When the user has granted standing restart authorization, recorded in the orchestration design in force, a pure coordinator parks the failed task instead of stopping the whole effort: the task carries a Blocked status whose reason states the failure summary and the attempt count, reported to the user without blocking the remaining work, and the coordinator re-dispatches parked tasks per the design's checkpoint-event rule. The restart parameters (restart riding, oldest-cleared-first ordering, backoff interval, and escalation bounds) are those the orchestration design in force records, which for this repository is manager.md rule 6 per opencode/DESIGN.md. The stop-report-ask path above remains the default whenever no such grant is recorded.

Every continuation of a failed attempt, including the single retry of a failed delegation, the restart of a parked task under a standing restart grant, and the direct takeover path described above, is bound by a single reconstruction duty and resumes from the work that survived the failed attempt instead of re-dispatching blind. A failed attempt is presumed to have produced partial work, so the continuation assumes work exists and reconstructs how much of it survived. Before re-dispatching or continuing personally, the implementation MUST reconstruct surviving work from durable evidence: the commits and the working-tree diff against the recorded base commit, the unit's worktree where worktree isolation was used, the Work Log entries and report file deposits where the task-files protocol is active, and the failure summary's own account of how far the attempt got.

The reconstruction reaches the work through one of two delivery paths, telling a worker (as a fresh dispatch or a resumed session) or executing the remainder personally. A fresh dispatch (a lost session, a poisoned context, or a cross-agent handoff) MUST carry the reconstruction in its prompt so the new worker verifies the remaining work rather than redoing it; a resumed same-worker session MUST be directed to reconcile its plan against the reconstructed evidence before continuing. A direct takeover MUST reconcile the plan the same way before executing the remainder personally. When the reconstruction shows the work already complete, the continuation MUST route the unit to verification, and to review where a review round applies, instead of re-dispatching any worker.
