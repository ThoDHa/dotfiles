...
- **Planning-mode exception.** The planner dispatched to plan a Triage task file MAY edit exactly the planning sections of that named task file: Objective, Success Criteria, Technical Approach, Risk Assessment, Testing Strategy, Task Breakdown, and Decision Log. The exception covers nothing else: header fields, acceptance-criteria checkboxes, Progress, and the dashboard stay manager-owned, no status transition (including Triage → Ready) belongs to the planner, and execution-phase writes return to the two CLI channels above.
