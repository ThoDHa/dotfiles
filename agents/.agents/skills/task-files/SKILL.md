...
This specification defines requirements for creating and managing task files during complex operations, in both Manager Mode (Delegating) and Manager Mode (Solo).
...
- User explicitly confirms task file tracking after being offered for a large task per [Large Task Offer](#large-task-offer)

Casual summary requests ("summarize", "quick recap") do NOT trigger task file creation.

Manager Mode activation is not a creation trigger: activating Manager Mode, explicitly or by choosing parallel delegation or worktree execution at the Task Complexity Protocol prompt, triggers only the offer of task-file tracking per the [Large Task Offer](#large-task-offer) gate, never silent creation, so an activation on a task below the gate's threshold promises no offer; task files are created only on the user's explicit confirmation.
...
└── .tasks/
    ...
    │   └── research/                             # Reserved subtree for task-file-less research deposits
    │       └── NN-<slug>.md
    └── archive/
...
A dispatched agent's `.tasks/` writes are exactly two CLI channels plus the research-deposit exception (the direct file write into the reserved `research/` namespace per the [Reports Namespace](#reports-namespace)); anything else is forbidden:
...
- Agents MUST NOT edit task files, header fields, the dashboard, or any other `.tasks/` artifact outside the exceptions this section defines: task-file writes belong to the manager, and dashboard mutations belong to the `tasks` command.
...
- **Research-deposit exception.** A research agent dispatched without a task file MAY write its report file directly at the canonical `research/` path under `.tasks/reports/` ([Reports Namespace](#reports-namespace)), the one case the two channels cannot serve because `tasks report` requires a resolvable task file. The exception covers that report file alone, and the write MUST NOT overwrite an existing file; every other `.tasks/` write returns to the two CLI channels above.
