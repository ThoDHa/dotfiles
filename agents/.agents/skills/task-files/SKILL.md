...
The manager's edits to task-file body prose are writer operations like any other and MUST go through `tasks edit <taskfile> --section <heading> [--append] [<file>|-]`, so the edit, the **Updated** refresh, and the dashboard render land as one serialized operation under the same board lock as `log` and `report`.
...
Sanctioned planning-worker edits are the one carve-out from this gate, defined once in the [Agent Write Path](#agent-write-path)'s planning-mode exception, which applies while its own dispatch is in flight; condition 2's flight check governs every other writer's timing, never the planner's own sanctioned edits.
